...
class SCNN(nn.Module):
    ...
    def __init__(self,cfg_cnn):
        ...
        self.fc1 = nn.Linear(64*16*16, cfg_fc[0], device=device)
        self.fc2 = nn.Linear(cfg_fc[0], cfg_fc[1], device=device)
    ...
    #     # convolutional layers membrane potential and spike memory
    #     # print('s',input.shape)
    #     c1_mem = c1_spike = torch.zeros(batch_size * 2, self.cfg_cnn[0][1], cfg_kernel[0], cfg_kernel[0], device=device)
    #     # print('s',c1_mem.shape)
    #
    #     c2_mem = c2_spike = torch.zeros(batch_size * 2, self.cfg_cnn[1][1], cfg_kernel[1], cfg_kernel[1], device=device)
    #     # print('s',c2_mem.shape)
    #
    #     # linear layers membrane potential and spike memory
    #     c3_mem = c3_spike = torch.zeros(batch_size * 2, self.cfg_cnn[2][1], cfg_kernel[2], cfg_kernel[2], device=device)
    #     # c4_mem = c4_spike = torch.zeros(batch_size * 2, self.cfg_cnn[2][2], cfg_kernel[3], cfg_kernel[3], device=device)
    #
