...
def extract_davis(aedat_file_path, filename_txt_path, croped_events_path, save_path_dvs, save_path_aps,
                  dvs_img_interval):
    frame_no = 0
    frame_all = []
    frame_exposure_time = []
    ...
    with AedatFile(aedat_file_path) as f:
        ...
        print('dvs_img_interval ',dvs_img_interval)
        # print('total',int(frame_num / dvs_img_interval) - 1)

        frame_count=int(frame_num / dvs_img_interval) - 1
        video=np.zeros((frame_count,height,width,3))
        out = cv2.VideoWriter('output_video.mp4', cv2.VideoWriter_fourcc(*'MP42'), 24, (height,width))

        for frame_no in range(0,frame_count):
            print('frame_no',frame_no)
            # event_frame = 255 * np.ones((height, width, 3), dtype=np.uint8)
            event_frame = 128 * np.ones((height, width), dtype=np.uint8)
            ...
            event_frame[event['y'][on_idx], event['x'][on_idx]] = [255]
            event_frame[event['y'][off_idx], event['x'][off_idx]] = [0]

            # cv2.imwrite(os.path.join(save_path_dvs, 'frame{:04d}'.format(frame_no * dvs_img_interval) + '.bmp'),
            #             event_frame)
            # cv2.imshow('Event Image', event_frame)
            # cv2.waitKey(0)
            # print('The {} timestamp is {}'.format(frame_no, events['timestamp'][frame_no]))
            file.write('The {} frame, the timestamp is {}'.format(frame_no, events['timestamp'][
                frame_no]) + "\n")  # save the timestamp
...
def read():
    ...
    ppp='/Users/haroldfitz/Desktop/The only folder on desktop/dataset/test_subset'
    print(ppp)
    ...
    for root, dirs, files in os.walk(aedat_path):
        ...
        for file in files:
            ...
            if (extension == '.aedat4'):
                ...
                extract_davis(aedat_file_path, filename_txt_path, croped_events_path, save_path_dvs, save_path_aps,
                              dvs_img_interval)
